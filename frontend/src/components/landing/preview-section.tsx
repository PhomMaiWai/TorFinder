      ...
      <div className="mx-auto max-w-[1000px] px-6">
        ...
        <div className="rounded-2xl border border-border bg-surface p-2 shadow-sm sm:p-3">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <Link
              href="/public"
              className="hidden h-10 shrink-0 items-center justify-center rounded-lg bg-ink px-5 text-sm font-semibold text-surface transition-colors hover:opacity-90 sm:flex"
            >
              ค้นหาทั้งหมด
            </Link>
          </div>
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {isUrgent && (
                      <span className="flex items-center gap-1.5 rounded-full bg-danger-soft px-2.5 py-1 text-[11px] font-semibold text-danger">
                        <div className="size-1.5 animate-pulse rounded-full bg-danger" />
                        ใกล้หมดเขต
                      </span>
                    )}
                  </div>
                  ...
                  <Link href={`/tor/${tor.id}`} className="mt-3 block group-hover:cursor-pointer">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px] text-ink-muted">
                      ...
                      <span className="font-medium text-ink">{tor.budget}</span>
                      <span
                        className={`flex items-center gap-1.5 ${
                          isUrgent ? "font-medium text-danger" : ""
                        }`}
                      >
                        <Clock size={15} className={isUrgent ? "text-danger" : "text-ink-subtle"} />
                  ...
                  <div className="flex items-center gap-2">
                    {isFeedbackStage && (
                      <button
                        ...
                        className="flex h-8 items-center gap-1.5 rounded-lg bg-surface-alt px-3 text-xs font-medium text-ink transition-colors hover:bg-border"
                      >
                    ...
                    <Link
                      href={`/tor/${tor.id}`}
                      className="flex h-8 items-center gap-1.5 rounded-lg bg-ink px-3 text-xs font-medium text-surface transition-colors hover:opacity-90"
                    >
                      ดูรายละเอียด
                    </Link>
                  </div>
        ...
        {items.length === 0 && (
          <div className="mt-6 rounded-2xl border border-dashed border-border bg-surface py-16 text-center">
            <p className="text-[15px] font-medium text-ink">ไม่พบโครงการที่ตรงกับคำค้นหา</p>
          </div>
        )}
        ...
        <div className="mt-10 text-center">
          <Link
            href="/public"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-lg border border-border bg-surface px-6 text-[15px] font-medium text-ink shadow-sm transition-all hover:bg-surface-alt hover:text-ink"
          >
