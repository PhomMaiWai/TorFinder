...
export default async function AdminAuditPage() {
  const [entries, t] = await Promise.all([fetchActivity(), getTranslations("AdminAuditPage")]);
          ...
          <div className="divide-y divide-border">
            {entries.map((log) => (
              ...
              >
                ...
                >
                  {log.kind === "auto" ? t("typeAuto") : t("typeManual")}
                </span>
                ...
                <div className="shrink-0 text-right max-sm:text-left">
                  ...
                  <p className="text-xs text-ink-muted">{thaiDateTime(log.createdAt)}</p>
                </div>
              </div>
            ))}
          </div>
