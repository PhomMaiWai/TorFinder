// End-to-end coverage for the data.go.th (กรมบัญชีกลาง) import (backend/src/datagov),
// run in CI by .github/workflows/auth.yml against a freshly booted backend + MongoDB.
//
// Same approach as tor-integration.e2e.test.mjs's e-GP coverage: Node's built-in
// runner + fetch over HTTP, admin-guard checks always on, and the live portal
// fetch kept opt-in since it reaches the real data.go.th CKAN API.
...
const ADMIN_TOKEN = makeToken("admin", "datagov-admin@ci.test");
const ORG_TOKEN = makeToken("org", "datagov-org@ci.test");
...
describe("POST /api/datagov/sync — admin only", () => {
  it("forbids a request with no token (403)", async () => {
    const res = await api("/datagov/sync", { method: "POST" });
    assert.equal(res.status, 403, "starting an import must take an admin session");
  });

  it("forbids an organization token (403)", async () => {
    const res = await api("/datagov/sync", { method: "POST", token: ORG_TOKEN });
    assert.equal(res.status, 403);
  });
});

describe("POST /api/datagov/sync — live import contract (opt-in)", () => {
  // Off in CI: this reaches the real data.go.th CKAN API. Run locally with
  // E2E_DATAGOV_LIVE=1 to exercise it.
  const skip = process.env.E2E_DATAGOV_LIVE !== "1";

  it("is routed and answers with a sync result or a portal-unavailable error", { skip, timeout: 180_000 }, async () => {
    const res = await api("/datagov/sync", { method: "POST", token: ADMIN_TOKEN });
